/**
 * 13:55 PM CDMX, 10-minute window at smwhr HQ.
 ...
 *
 * Usage:
 *   cd apps/api && pnpm db:add-1355-quick
 */
...
// 13:55 – 14:05 CDMX today (UTC-6 → 19:55–20:05 UTC).
function quickWindowToday(): { startsAt: Date; endsAt: Date } {
  const now = new Date();
  const y = now.getUTCFullYear();
  const m = now.getUTCMonth();
  const d = now.getUTCDate();
  const startsAt = new Date(Date.UTC(y, m, d, 19, 55, 0));
  const endsAt = new Date(Date.UTC(y, m, d, 20, 5, 0));
...
const event: FutureEventInput = {
  slug: 'smwhr-hq-1355-quick',
  title: 'smwhr hq · 13:55 quick',
  venueName: 'smwhr HQ',
  city: 'Tulancingo',
  country: 'MX',
  category: 'culture',
